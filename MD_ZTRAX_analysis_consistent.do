...
***************************************************************************
*     Process of assessment, sales, and analysis of Montgomery County     *
***************************************************************************
...
**************************************************
*           Data with Treatment-MONTGOMERY       *
**************************************************
*The flight path is intersected with the ZTRAX points in ArcGIS
set more off
use "$dta0\ztrax_treat6km.dta",clear
set seed 1234567

merge 1:1 propertyfu propertyci importparc using"$dta0\ztrax_treat5km.dta"
gen Buffer_5km=(_merge==3)
drop _merge
merge 1:1 propertyfu propertyci importparc using"$dta0\ztrax_treat4km.dta"
gen Buffer_4km=(_merge==3)
drop _merge
merge 1:1 propertyfu propertyci importparc using"$dta0\ztrax_treat3km.dta"
gen Buffer_3km=(_merge==3)
drop _merge
merge 1:1 propertyfu propertyci importparc using"$dta0\ztrax_treat2km.dta"
gen Buffer_2km=(_merge==3)
drop _merge
merge 1:1 propertyfu propertyci importparc using"$dta0\ztrax_treat1km.dta"
...
use "$dta0\ZTRAX_lowelevsec.dta",clear
ren propertyfu PropertyFullStreetAddress
ren propertyci PropertyCity
ren importparc ImportParcelID
ren latfixed LatFixed
...
ren county County
ren legaltowns LegalTownship
save "$dta0\ZTRAX_lowelevsec.dta",replace
*/
clear all
set more off
set max_memory 16g
...
drop if _merge==2
*73240 out of 145,566 properties find no transaction
drop _merge
...
esttab DID_linear DID_ln DID_linear1km DID_ln1km DID_linear2km DID_ln2km DID_linear3km DID_ln3km using"$results\DID_results.csv", keep(1.Treatment 1.post 1.Treatment#1.post 1.Treatment1km 1.Treatment1km#1.post 1.Treatment2km 1.Treatment2km#1.post 1.Treatment3km 1.Treatment3km#1.post) replace b(a3) se r2(3) star(+ .1 * .05 ** .01 *** .001) stats (N N_g r2, fmt(0 3))
esttab DID_linear DID_ln DID_linear1km DID_ln1km DID_linear2km DID_ln2km DID_linear3km DID_ln3km using"$results\DID_results.html", keep(1.Treatment 1.post 1.Treatment#1.post 1.Treatment1km 1.Treatment1km#1.post 1.Treatment2km 1.Treatment2km#1.post 1.Treatment3km 1.Treatment3km#1.post) replace b(a3) se r2(3) star(+ .1 * .05 ** .01 *** .001) stats (N N_g r2, fmt(0 3))
...
esttab Xt_linear_consist Xt_ln_consist Xt_linear1km Xt_ln1km Xt_linear2km Xt_ln2km using"$results\Xt_results.csv", keep(post_treat Ring_*kmpost post_treat*km) replace b(a3) se r2(3) star(+ .1 * .05 ** .01 *** .001) stats (N N_g r2, fmt(0 3))
esttab Xt_linear_consist Xt_ln_consist Xt_linear1km Xt_ln1km Xt_linear2km Xt_ln2km using"$results\Xt_results.html", keep(post_treat Ring_*kmpost post_treat*km) replace b(a3) se r2(3) star(+ .1 * .05 ** .01 *** .001) stats (N N_g r2, fmt(0 3))

tab post_treat
duplicates tag PID if Treatment==1,gen(dup_sale_treat)
count if post_treat==1&dup_sale_treat>=1
*for xt regs, 70 treated have duplicated sales that are utilized in the identification for post_treat
*Comparison: DID utilizes 389 
...
drop dup_Ring_*
*For xt regressions:
*69 properties have duplicated sales that are utilized in the identification for 1km buffer
*48 for 2km buffer
*50 for 3km buffer
*65 for 4km buffer
*45 for 5km buffer
...
merge m:1 ImportParcelID using "$dta0\ZTRAX_lowelevsec.dta"
drop if _merge!=3
...
esttab DID_linear_lowelev DID_ln_lowelev DID_linear1km_lowelev DID_ln1km_lowelev DID_linear2km_lowelev DID_ln2km_lowelev DID_linear3km_lowelev DID_ln3km_lowelev using"$results\DID_results_midelev.csv", keep(1.Treatment 1.post 1.Treatment#1.post 1.Treatment1km 1.Treatment1km#1.post 1.Treatment2km 1.Treatment2km#1.post 1.Treatment3km 1.Treatment3km#1.post) replace b(a3) se r2(3) star(+ .1 * .05 ** .01 *** .001) stats (N N_g r2, fmt(0 3))
esttab DID_linear_lowelev DID_ln_lowelev DID_linear1km_lowelev DID_ln1km_lowelev DID_linear2km_lowelev DID_ln2km_lowelev DID_linear3km_lowelev DID_ln3km_lowelev using"$results\DID_results_midelev.html", keep(1.Treatment 1.post 1.Treatment#1.post 1.Treatment1km 1.Treatment1km#1.post 1.Treatment2km 1.Treatment2km#1.post 1.Treatment3km 1.Treatment3km#1.post) replace b(a3) se r2(3) star(+ .1 * .05 ** .01 *** .001) stats (N N_g r2, fmt(0 3))
...
esttab Xt_linear_consist_lowelev Xt_ln_consist_lowelev Xt_linear1km_lowelev Xt_ln1km_lowelev Xt_linear2km_lowelev Xt_ln2km_lowelev using"$results\Xt_results_midelev.csv", keep(post_treat Ring_*kmpost post_treat*km) replace b(a3) se r2(3) star(+ .1 * .05 ** .01 *** .001) stats (N N_g r2, fmt(0 3))
esttab Xt_linear_consist_lowelev Xt_ln_consist_lowelev Xt_linear1km_lowelev Xt_ln1km_lowelev Xt_linear2km_lowelev Xt_ln2km_lowelev using"$results\Xt_results_midelev.html", keep(post_treat Ring_*kmpost post_treat*km) replace b(a3) se r2(3) star(+ .1 * .05 ** .01 *** .001) stats (N N_g r2, fmt(0 3))

tab post_treat
duplicates tag PID if Treatment==1,gen(dup_sale_treat)
count if post_treat==1&dup_sale_treat>=1
*33 treated have duplicated sales that are utilized in the identification for post_treat
*Comparison: DID utilizes 203
...
*26 properties have duplicated sales that are utilized in the identification for 1km buffer
*18 for 2km buffer
*24 for 3km buffer
*18 for 4km buffer
*16 for 5km buffer
