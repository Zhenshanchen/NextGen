...
**************************************************************************
*   Do file- Process of sales, and analysis of Fairfox  County     *
**************************************************************************
clear all
set more off
cap log close
...
**************************************************
*           Data with Treatment-FF               *
**************************************************
*The flight path is intersected with the ZTRAX points in ArcGIS
set more off
use "$dta0\ztrax_treat6km_ffva.dta",clear
set seed 1234567

merge 1:1 propertyfu propertyci importparc using"$dta0\ztrax_treat5km_ffva.dta"
gen Buffer_5km=(_merge==3)
drop _merge
merge 1:1 propertyfu propertyci importparc using"$dta0\ztrax_treat4km_ffva.dta"
gen Buffer_4km=(_merge==3)
drop _merge
merge 1:1 propertyfu propertyci importparc using"$dta0\ztrax_treat3km_ffva.dta"
gen Buffer_3km=(_merge==3)
drop _merge
merge 1:1 propertyfu propertyci importparc using"$dta0\ztrax_treat2km_ffva.dta"
gen Buffer_2km=(_merge==3)
drop _merge
merge 1:1 propertyfu propertyci importparc using"$dta0\ztrax_treat1km_ffva.dta"
gen Buffer_1km=(_merge==3)
drop _merge
merge 1:1 propertyfu propertyci importparc using"$dta0\ztrax_treat_ffva.dta"
...
save "$dta0\ZTRAX_oneunit_withtreat_FFVA.dta",replace

***********************************************
*           Processing Transaction            *
***********************************************
use "$dta0\ZTRAX_oneunit_withtreat_FFVA.dta",clear
...
save "$dta0\ZTRAX_oneunitValue_withtreat_FFVA.dta",replace
...
set max_memory 16g
set matsize 11000
use "$Zitrax\dta\transaction_51.dta",clear
set seed 1234567
drop if FIPS!=51059
merge 1:m TransId using"$Zitrax\dta\transaction_property_51.dta",keepusing(FIPS AssessorParcelNumber ImportParcelID PropertyFullStreetAddress PropertyCity)
drop if _merge!=3
...
/*restriction 1339+21140 dropped*/
...
/*restriction 369,683 dropped*/
tab DataClassStndCode, sum(LoanAmount)

*417,375 deed records without mortgage, 84,588 finance records (deed with cocurrent mortgage)
...
/*restriction 53,213 deed records dropped*/
...
merge 1:m TransId using"$Zitrax\dta\transaction_buyer_51.dta",keepusing(BuyerLastName BuyerIndividualFullName BuyerNonIndividualName)
drop if _merge==2
drop _merge
set seed 123445565
sort *
...
joinby TransId using"$Zitrax\dta\transaction_seller_51.dta",unmatched(master)
drop v7 v8 v9 v10 
...
joinby ImportParcelID using"$dta0\ZTRAX_oneunitValue_withtreat_FFVA.dta",unmatched(master)
...
drop matched_PID
/*restriction  dropped-dropping those will be not matched with property data (outside the study region - 2 miles from the flight path)*/
...
save "$dta0\sales_nonarmsprocessed_FFVA.dta",replace

**********************************************************
*       Merge Assess with Transaction & Analysis         *
**********************************************************
*one-time rename dta file transformed from ArcGIS
use "$dta0\ztrax_lowelevsec6kft_ffva.dta",clear
ren propertyfu PropertyFullStreetAddress
ren propertyci PropertyCity
ren importparc ImportParcelID
ren latfixed LatFixed
...
ren county County
ren legaltowns LegalTownship
save "$dta0\ztrax_lowelevsec6kft_ffva.dta",replace
*
...
set max_memory 16g
set matsize 11000
use "$dta0\sales_nonarmsprocessed_FFVA.dta",clear
merge m:1 ImportParcelID using "$dta0\ZTRAX_oneunit_withtreat_FFVA.dta"
drop if _merge==2
*24，461 out of 62,847 properties find no transaction
drop _merge
...
esttab DID_linear DID_ln DID_linear1km DID_ln1km DID_linear2km DID_ln2km DID_linear3km DID_ln3km using"$results\DID_results_FFVA.csv", keep(1.Treatment 1.post 1.Treatment#1.post 1.Treatment1km 1.Treatment1km#1.post 1.Treatment2km 1.Treatment2km#1.post 1.Treatment3km 1.Treatment3km#1.post) replace b(a3) se r2(3) star(+ .1 * .05 ** .01 *** .001) stats (N N_g r2, fmt(0 3))
esttab DID_linear DID_ln DID_linear1km DID_ln1km DID_linear2km DID_ln2km DID_linear3km DID_ln3km using"$results\DID_results_FFVA.html", keep(1.Treatment 1.post 1.Treatment#1.post 1.Treatment1km 1.Treatment1km#1.post 1.Treatment2km 1.Treatment2km#1.post 1.Treatment3km 1.Treatment3km#1.post) replace b(a3) se r2(3) star(+ .1 * .05 ** .01 *** .001) stats (N N_g r2, fmt(0 3))
...
esttab Xt_linear_consist Xt_ln_consist Xt_linear1km Xt_ln1km Xt_linear2km Xt_ln2km using"$results\Xt_results_FFVA.csv", keep(post_treat Ring_*kmpost post_treat*km) replace b(a3) se r2(3) star(+ .1 * .05 ** .01 *** .001) stats (N N_g r2, fmt(0 3))
esttab Xt_linear_consist Xt_ln_consist Xt_linear1km Xt_ln1km Xt_linear2km Xt_ln2km using"$results\Xt_results_FFVA.html", keep(post_treat Ring_*kmpost post_treat*km) replace b(a3) se r2(3) star(+ .1 * .05 ** .01 *** .001) stats (N N_g r2, fmt(0 3))

tab post_treat
duplicates tag PID if Treatment==1,gen(dup_sale_treat)
count if post_treat==1&dup_sale_treat>=1
*for xt regs, 80 treated have duplicated sales that are utilized in the identification for post_treat
...
drop dup_Ring_*
*For xt regressions:
*38 properties have duplicated sales that are utilized in the identification for 1km buffer
*40 for 2km buffer
*42 for 3km buffer
*40 for 4km buffer
*26 for 5km buffer
...
merge m:1 ImportParcelID using "$dta0\ztrax_lowelevsec6kft_ffva.dta"
drop if _merge!=3
...
esttab DID_linear_lowelev DID_ln_lowelev DID_linear1km_lowelev DID_ln1km_lowelev DID_linear2km_lowelev DID_ln2km_lowelev DID_linear3km_lowelev DID_ln3km_lowelev using"$results\DID_results_midelev_FFVA.csv", keep(1.Treatment 1.post 1.Treatment#1.post 1.Treatment1km 1.Treatment1km#1.post 1.Treatment2km 1.Treatment2km#1.post 1.Treatment3km 1.Treatment3km#1.post) replace b(a3) se r2(3) star(+ .1 * .05 ** .01 *** .001) stats (N N_g r2, fmt(0 3))
esttab DID_linear_lowelev DID_ln_lowelev DID_linear1km_lowelev DID_ln1km_lowelev DID_linear2km_lowelev DID_ln2km_lowelev DID_linear3km_lowelev DID_ln3km_lowelev using"$results\DID_results_midelev_FFVA.html", keep(1.Treatment 1.post 1.Treatment#1.post 1.Treatment1km 1.Treatment1km#1.post 1.Treatment2km 1.Treatment2km#1.post 1.Treatment3km 1.Treatment3km#1.post) replace b(a3) se r2(3) star(+ .1 * .05 ** .01 *** .001) stats (N N_g r2, fmt(0 3))
...
esttab Xt_linear_consist_lowelev Xt_ln_consist_lowelev Xt_linear1km_lowelev Xt_ln1km_lowelev Xt_linear2km_lowelev Xt_ln2km_lowelev using"$results\Xt_results_midelev_FFVA.csv", keep(post_treat Ring_*kmpost post_treat*km) replace b(a3) se r2(3) star(+ .1 * .05 ** .01 *** .001) stats (N N_g r2, fmt(0 3))
esttab Xt_linear_consist_lowelev Xt_ln_consist_lowelev Xt_linear1km_lowelev Xt_ln1km_lowelev Xt_linear2km_lowelev Xt_ln2km_lowelev using"$results\Xt_results_midelev_FFVA.html", keep(post_treat Ring_*kmpost post_treat*km) replace b(a3) se r2(3) star(+ .1 * .05 ** .01 *** .001) stats (N N_g r2, fmt(0 3))

tab post_treat
duplicates tag PID if Treatment==1,gen(dup_sale_treat)
count if post_treat==1&dup_sale_treat>=1
*50 treated have duplicated sales that are utilized in the identification for post_treat
*Comparison: DID utilizes 
...
*2 for 4km buffer
*8 for 5km buffer
*Note: Fairfox has very frenquent flight noise.
